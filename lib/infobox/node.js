define(["moment", "numeral", "tablesort", "tablesort.numeric"],
  ...
  function showSiteCode(d) {
    return dictGet(d.nodeinfo, ["system", "site_code"])
  }

  function showClientsc(d) {
    if (!d.flags.online)
      return undefined

    return function (el) {
      var countClientsMesh = 0
       d.neighbours.forEach( function (d) {
      countClientsMesh += d.node.statistics.clients
      return countClientsMesh
    })

      el.appendChild(document.createTextNode(d.statistics.clients + countClientsMesh))
      // el.appendChild(document.createTextNode(d.statistics.clients > 0 ? d.statistics.clients : "keine"))
      el.appendChild(document.createElement("br"))

      var span = document.createElement("span")
      span.classList.add("clients")
      span.textContent = " ".repeat(d.statistics.clients + countClientsMesh)
      el.appendChild(span)
    }
  }
  ...
  return function(config, el, router, d) {
    ...
    attributeEntry(attributes, "Site-Code", showSiteCode(d))
    ...
    attributeEntry(attributes, "Clients im lokalen Mesh", showClientsc(d))
    ...
    if (d.neighbours.length > 0) {
      ...
      d.neighbours.forEach( function (d) {
        ...
        a4.href = "#"
        // a4.textContent = d.node.nodeinfo.clients
        a4.textContent = d.node.statistics.clients
        // a4.onclick = router.link(d.link)
        td4.appendChild(a4)
        // td4.setAttribute("data-sort", d.statistics.clients !== undefined ? -d.statistics.clients : 1)
        tr.appendChild(td4)
